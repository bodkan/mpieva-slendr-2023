...
![](images/dna_sequence.png){fig-align="center" width="50%"}

<br>...a linear sequence of nucleotides...
...
<center>***This could be a simple vector of letters/numbers in R.***</center>
...
<center>***This could be a a list of "chromosome vectors" in R...***</center>

. . .

<br>

<center>***... evolving (in a for loop), from generation to generation.***</center>
...
<center>**This is why we will focus on [*slendr*](http://www.slendr.net),**<br>a convenient R interface to both SLiM and _msprime_.</center>
:::
...
## Remember that _slendr_ objects carry their history
...
<br><center>(A tree sequence can be saved to disk with `ts_save()`.)</center>
...
## _slendr_'s R interface to [_tskit_](https://tskit.dev/tskit)
...
## In fact, we don't even _need_ mutations!
...
## Exercise #2 &mdash; estimating $N_e$ from AFS
...
1.  Write an R function (in a new script) that gets $N_e$ as input, creates a _slendr_ population, compiles a model, simulates a tree sequence, runs `ts_afs()` on it, and returns the AFS.
...
## Exercise #3 &mdash; adding `gene_flow()`
...
## Exercise #4 &mdash; popgen statistics
...
A useful trick for quickly working with samples from entire populations:
...
So far, everything we did used "present-day genomes" (even the archaics!).

To record _ancient_ individuals in the tree sequence, we can use the function `schedule_sampling()`.
